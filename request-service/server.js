...
app.post('/create-request', verifyToken, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      // Notify both user and superior by email
      sendEmail(user_email, 'Request Sent', 'Your request has been sent.')
      sendEmail(
        superior_email,
        'New Request Assigned',
        'A new request requires your approval.'
...
// Load SSL certificate and key files, with fallback to HTTP if they don’t exist
const sslOptions = {
  key: fs.existsSync(path.join(__dirname, '../server.key'))
    ? fs.readFileSync(path.join(__dirname, '../server.key'))
    : null,
  cert: fs.existsSync(path.join(__dirname, '../server.cert'))
    ? fs.readFileSync(path.join(__dirname, '../server.cert'))
    : null
}

// Start HTTPS server if certificates are available, otherwise use HTTP
const PORT = process.env.PORT || 3002
if (sslOptions.key && sslOptions.cert) {
  https.createServer(sslOptions, app).listen(PORT, () => {
    console.log(`Secure request service running on https://localhost:${PORT}`)
  })
} else {
